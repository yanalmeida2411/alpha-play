"use server";

import { getToken } from "./profileAction";
import { ActionResponse } from "../types";

const API_URL = process.env.NEXT_PUBLIC_API_URL || "";

export interface MemoryGamePayload {
  childId: string;
  timeToStart: string;
  timeToFinish: string;
  wrongMatches: number;
  correctMatches: number;
}

export interface MemoryGameSession {
  earnedStars: number;
}

export async function saveMemoryGameResult(
  payload: MemoryGamePayload,
): Promise<ActionResponse<MemoryGameSession>> {
  const token = await getToken();

  if (!token) return { success: false, error: "Não autenticado" };

  try {
    const res = await fetch(`${API_URL}/games/memory-game`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify(payload),
    });

    const data = await res.json().catch(() => ({}));

    if (!res.ok) {
      console.error("Erro API memory game:", data);
      return {
        success: false,
        error: data.message || "Erro ao salvar resultado do jogo",
      };
    }

    return { success: true, data: data.data };
  } catch (error) {
    console.error("Erro memory game:", error);
    return { success: false, error: "Erro de conexão ao salvar resultado" };
  }
}
